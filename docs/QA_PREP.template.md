...
**English only; question-shaped text; attack styles far from anything we trained on.** The Attack lab shows two on purpose: an override in French, and a genuine question inside an email. It also misses a hidden "recommend this lender" line. We list limitations on the About page and the last slide.
