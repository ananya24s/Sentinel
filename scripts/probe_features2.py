"""Gullible-agent influence features (v2) for the scorer's gray-zone spans and cache the features.
...
def main():
    ...
    out_path = CACHE / f"probe2_{split}.jsonl"
    done = set()
    if out_path.exists():
    ...
    with open(out_path, "a") as f:
        for n, (d, gray) in enumerate(jobs):
            ...
            if not todo:
                continue
            A = probe.answer(d["task"], d["text"], __import__('sentinel.probe', fromlist=['GULLIBLE']).GULLIBLE)
            for i in todo:
                s = d["spans"][i]
                r = probe.influence2(d["task"], d["text"], s["start"], s["end"], s["text"], A)
